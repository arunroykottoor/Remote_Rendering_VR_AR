...
void Game::save_image(std::vector<unsigned char>* buffer) {
	char filepath[] = "C:/git/gump/3D_Engine/Images/test/2.png";
	...
	//std::cout << "Buffer size: " << bufferSize << std::endl;
	glReadPixels(0, 0, framebufferWidth, framebufferHeight, GL_BGR, GL_UNSIGNED_BYTE, buffer->data()); // GL_BGR GL_RGB
	//std::cout << "bffer.size inside " << buffer->size() << std::endl;
	SOIL_save_image(filepath, SOIL_SAVE_TYPE_BMP, framebufferWidth, framebufferHeight, nrChannels, buffer->data());
